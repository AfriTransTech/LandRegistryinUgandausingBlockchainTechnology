...
print("LAND REGISTRY SYSTEM IN UGANDA USING BLOCKCHAIN TECHNOLOGY")
import json
...
properties.append({'property_address' : 'land area : 3, Plot no : 7, Street no : 4, kawempe', 'property_owner' : 'joan', 'property_owner_id' : 42310236846167})
...
participant = OrderedDict(
    [ ('name', 'stevo'), ('id', 12345), ('properties', []), ('past_properties', []) ])
...
property1 = OrderedDict(
        [('property_address', 'land area : 3, Plot no : 7, Street no : 60,  kawempe'), ('property_owner', 'joan'), ('property_owner_id', 42310236846167)])

properties.append(property1)

properties.append(
        OrderedDict(
        [('property_address', 'land area : 6, Plot no : 13, Street no : 20, mukono'), ('property_owner', 'Denis'), ('property_owner_id', 42310236846168)
        ])
)
...
owner_details = OrderedDict(   #change name to owner_database
    [('name', 'Denis'), ('id', 42310236846168), ('properties', []), ('past_properties', [])
    ]
)
...
#--------------------------------
"""
properties.append({'property_address' : 'land no : 3, Plot no : 7, Street no : 40, kawempe Area', 'property_owner' : 'joan', 'property_owner_id' : 42310236846167})
properties.append({'property_address' : 'Flat no : 6, Plot no : 3, Street no : 41, mukono Area', 'property_owner' : 'Denis', 'property_owner_id' : 42310236846168})
...
def registering_property():
    ...
    print('enter the number of Land you want to choose')
    ...
    if user_choice >= 0 and user_choice < len(govt_database['properties']):
        ...
        if verify_property_owner(owner_details, property_details):
            print('Land verified successfully ')
            ...
            print('land not verified successfully ')
            return False
    else:
...
def transfer_property():
    ...
    if i==-1:
        print('You dont have any land registered under your names so you cannot transfer, please register')
    else:
        print('enter the number of land you want to choose')
        user_choice = get_user_choice_int()
        print(' list owner_land[properties] = ', len(owner_details['properties']))
        #Add a while loop which will prompt values again and again if user inputs wrong value, press -1 to exit
        if user_choice >= 0 and user_choice < len(owner_details['properties']):
            ...
            if x != -1:
                ...
                if add_transaction(recipient_details, owner_details, property_details, amount, user_choice):
                    print('land title transfered successfully ')
                    return True
                else:
                    print('land title transfer failed ')
                    return False
            elif x == -1:
                print('recipient not registered, ask him or her to sign up first')
                return False
        else:
            print('user choice is invalid')
            return False
...
def print_user_details():
    ...
    print('user land: ', owner_details['properties'])
